...
- Validated baseline (2026-08-04, stage-2 load-bearing validation): `./scripts/linux-tests.sh` at HEAD `01ca94b` = exit 0 + `LINUX SUITE: GREEN`, 1667 tests across 9 projects, 0 failed/0 errors (log: `/home/dan/code/winpepper/.worktrees/.the-usual-logs/misheard-newest-first/reports/a5-linux-baseline.log`). Task 1 Step 2's "all pre-existing tests pass" expectation is anchored to this run. Windows-gate reachability from this WSL session was also verified (transport + Windows toolchain + worktree visibility probes all passed); gate baseline greenness is deliberately deferred to Task 4's failure handling.
- Before push: `./scripts/windows-gate.sh` (run from WSL) must exit 0 and print `GATE: GREEN` (~40 min; allow a 3600s timeout). Never run it concurrently with `linux-tests.sh`; never mix Linux and Windows builds in the same `bin/`/`obj/`. Cleaning is ASYMMETRIC (verified by stage-2 script inspection + timed runs): `windows-gate.sh` pre-cleans `src/**` and `tests/**` `bin`/`obj` before its Windows builds, but `linux-tests.sh` builds INCREMENTALLY with no clean step — so after any `windows-gate.sh` run (e.g. Task 4's fix-and-retry loop), delete `src/**` and `tests/**` `bin`/`obj` yourself before the next `linux-tests.sh` run.
...
**Known, unchanged limitation:** the Corrections VM is long-lived and does not re-seed on page navigation (`CorrectionsPage.xaml.cs` `OnNavigatedTo` reuses `AppShell.CorrectionsVm`), so an entry learned via the post-paste toast appears in the list only after app relaunch — exactly as today. It then appears at the TOP (it is last in the file = newest). This plan does not alter that pre-existing behavior. Also unchanged: re-learning an EXISTING wrong→right mapping overwrites the value in place and keeps the entry's original file position — "newest" means most recently FIRST-added, not most recently re-confirmed (shipped semantics since v0.6.0-alpha; verified by stage-2 git-history validation).
...
(`Reverse()` is `System.Linq.Enumerable.Reverse` — unambiguous on `ObservableCollection<T>`, which has no instance `Reverse`. Do NOT touch the `Preferred` seed loop, `AddPreferred`, or the `Preferred` half of `Persist()`. Mutate `Replacements` only via `Insert`/`Remove` — never `Clear()`-and-rebuild: per-index INCC Add/Remove is docs-verified for the bound ListView, while Reset/`Clear()` has a known WinUI issue in some layouts, microsoft-ui-xaml #8684.)
...
Expected: **PASS on first run.** This is deliberately a verification/pinning test, not a red-first test: Task 1 already implemented the behavior at the VM seam; what this test adds is empirical proof of the JSON⇄Dictionary ordering assumption against a real file. **If it FAILS, halt** — do not adjust assertions to match observed order. A failure means the round-trip does not preserve document order, "newest-first after relaunch" is broken, and the fix (an explicitly ordered persisted structure) is a schema change that is out of scope per Global Constraints; surface it as a blocker instead. (Stage-2 load-bearing validation already confirmed this chain empirically on this machine: the real `CorrectionStore` source, run on .NET 9.0.0 in a scratch project, preserved order through all four checks incl. the `Reverse().ToDictionary` step — see `/home/dan/code/winpepper/.worktrees/.the-usual-logs/misheard-newest-first/reports/validator-A1.md`. A failure here therefore most likely indicates a test-setup mistake, but the halt rule still stands.)
...
2. Add `chat gbt` → `ChatGPT`, then `ann thropic` → `Anthropic`. Each new entry appears at the TOP of the list; final order top-to-bottom: `ann thropic`, `chat gbt`. Also observe that the page's scroll position stays sane when an entry is prepended (cosmetic scroll-anchoring check).
