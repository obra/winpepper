...
- **D5 — destructive ops use a strict load or no load at all, a truthful delete protocol, and never throw for index/enumeration IO failures** (finder F2–F4, reviewer r1#5/r1#7/r2#2/r2#4).
  ...
  - `DeleteAllAudio()`'s payload action is sweeping `*.wav` recursively under the history root and needs NO index: it sweeps always (even with a corrupt index — the privacy intent is file deletion), tracks per-file success/failure, and THEN, only when the strict loader reads the index cleanly, clears `WavRelativePath` on entries whose file is gone-or-absent and saves once. A failed index save (or any enumeration failure) is CAUGHT and reported — never thrown into the UI chain: `DeleteAllAudio` returns `HistoryAudioCleanupResult { DeletedCount, FailedCount, IndexSaveFailed }`; `Prune` returns `HistoryPruneResult { DroppedCount, IndexSaveFailed }`. Re-runnable: a second call mops up what a failed first call kept.
...
- **D13 — commit chains are serialized** (reviewer r2#5). The VM funnels every setter's commit chain through one `SemaphoreSlim(1,1)`, so flush/prune/refresh/`RetentionApplied` complete in setter order; a quick burst can't leave stale notifications on top.
...
- Produces:
  ...
  - `public sealed record HistoryAudioCleanupResult { int DeletedCount; int FailedCount; bool IndexSaveFailed }` in `Winpepper.History`
...
- New pure-managed `HistoryRetentionViewModel` (in `Winpepper.History/ViewModels`): binds `StoreAudioEnabled` (bool), `MaxEntries` (double for NumberBox, clamped to [1,10000]), `MaxAgeDays` (double, clamped to [1,36500]; setting it while `KeepForever` is true STILL persists `null` — the invariant that protects D2), `KeepForever` (bool; true ⇒ persists `HistoryMaxAgeDays = null`), exposes `DiskUsageDisplay` and `LastCommitPersisted` (bool). The ctor populates `DiskUsageDisplay` immediately (and `Refresh()` re-reads it on demand). Each setter: (1) synchronously `slot.Publish(storeAudio, policy)` so the runtime gate flips NOW (D12); (2) `_ = CommitAndApplyAsync(mutator, committedPolicy)` — serialized through one `SemaphoreSlim(1,1)` (D13) — `LastCommitPersisted = await _writer.TryQueueAndFlushAsync(mutator);` → `await Task.Run(() => _store.Prune(committedPolicy));` → refresh `DiskUsageDisplay` (off-thread) → raise `RetentionApplied`.
...
- Produces:
  ...
  - `public sealed class HistoryRetentionViewModel : INotifyPropertyChanged`
    ...
    - `string DiskUsageDisplay` (populated in ctor + refreshed after each apply), `bool LastCommitPersisted` (read-only, notified)
...
- [ ] **Step 3: Add the minimal production implementation**
  ...
  ISettingsWriter: default member `async Task<bool> TryQueueAndFlushAsync(m) { await QueueAndFlushAsync(m); return true; }`. DebouncedSettingsWriter: factor the persisted/not-persisted outcome out of `Flush()` (persisted := nothing pending-or-Save-succeeded; not-persisted := degraded-load-kept or Save-failed-requeued) and implement the override end-to-end (Queue → cancel/await scheduled → flush → outcome). HistoryStore: `Prune(HistoryRetentionPolicy? policyOverride = null)`. New VM: ctor populates `DiskUsageDisplay` once; setters guard NaN/clamps (and keep `null` persisted while `KeepForever`), synchronously `slot.Publish(storeAudio, policy)`, then `_ = CommitAndApplyAsync(mutator, policy)` whose body is serialized via a private `SemaphoreSlim(1,1)`: `LastCommitPersisted = await _writer.TryQueueAndFlushAsync(mutator); await Task.Run(() => _store.Prune(policy)); await Task.Run(() => Refresh()); RetentionApplied?.Invoke(this, EventArgs.Empty);` inside try/finally releasing the semaphore; store-side failures never escape (D5) and any unexpected exception in the chain is swallowed into `LastCommitPersisted = false` so the fire-and-forget chain cannot fault unobserved (reviewer r2#2). `DeleteAllAudioAsync` wraps the sweep in `Task.Run`, refreshes usage, raises the event, returns the structured result. Detail VM: prepend the empty-path short-circuit and add `catch (FileNotFoundException)` in the transcription Runner.
